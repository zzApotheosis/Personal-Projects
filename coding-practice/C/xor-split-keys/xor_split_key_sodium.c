...
void split_key(const size_t key_len, const uint32_t n_parts, const uint8_t original_key[key_len], uint8_t split_keys[n_parts][key_len]) {
    ...
    for (uint32_t i = 1; i < n_parts; i++) {
        randombytes_buf(split_keys[i], key_len);
...
int main(int argc, char ** argv) {
    ...
    // Initialize sodium
    if (sodium_init() < 0) {
        fprintf(stderr, "sheeeeeit\n");
        exit(2);
    }

    // Generate a secure random key
    randombytes_buf(original_key, KEY_LEN);

    // Show that our original key is clearly defined in memory
    fprintf(stdout, "[*] HERE IS OUR ORIGINAL KEY GENERATED BY SODIUM\n");
