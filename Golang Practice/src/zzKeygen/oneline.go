...
func threeArgs(a []string) {
  ...
  length, err = strconv.Atoi(a[0]); util.Check(err)
  shift, err = strconv.Atoi(a[1]); util.Check(err)
  charset = a[2]
...
func fiveArgs(a []string) {
  ...
  // Parse arguments
  fullseed = a[0] + a[1]
  length, err = strconv.Atoi(a[2]); util.Check(err)
  shift, err = strconv.Atoi(a[3]); util.Check(err)
  charset = a[4]
