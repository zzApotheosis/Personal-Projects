...
//        }
        ...
        if (state == 0) {
            ...
            if (tempstate == 0) {
                if (selection == 0) { //Domestic
                    result = weight * 3.5;
                } else if (selection == 1) { //International
                    result = weight * 20;
                }
                tempstate = 1;
            }
                ...
                if (tmp > 10000) {
                    sprintf(lcdStr0, "Cost is");
                    sprintf(lcdStr1, "%.2f", result);
                    LCDClearLine(0);
                    LCDClearLine(1);
                    LCDWriteLine(lcdStr0, 0);
                    LCDWriteLine(lcdStr1, 1);
...
void interrupt HighISR(void) {
    ...
    if (INTCONbits.INT0IF) {
        ...
        if (!buttonPressed) {
            ...
            if (state == 2) { //Send reset signal
                WriteCANMsg(MSG3_ID, (void *) 0, 0, 0);
                tempstate = 0;
                state = 0;
            }
...
//            }
        ...
        if (id == MSG1_ID && state == 0) {
            selection = data[0];
            state = 1;
        } else if (id == MSG2_ID && state == 1) {
            weight = *((int *) data);
//            LATD = 0xff; //Poor man's debugger!
            state = 2;
        }
